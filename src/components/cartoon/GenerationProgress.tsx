...
const PHASES: Array<{ key: Exclude<GenerationPhase, null>; label: string; hint: string }> = [
...
const GenerationProgress: React.FC<GenerationProgressProps> = ({ phase, active }) => {
  ...
  const activeIndex = phase === 'script' ? 0 : phase === 'image' ? 1 : -1;
  const currentHint =
    activeIndex >= 0 ? PHASES[activeIndex].hint : 'Finishing up…';
